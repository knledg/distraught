...
exports.addCache = function(
  ...
  cache[name] = {
    scan(
      ...
      return new Promise((resolve, reject) => {
        ...
        client.scan(
          ...
          (
            err,
            [newCursor, newArrayOfKeys]: {
              newCursor: string,
              newArrayOfKeys: Array<string>,
            }
          ) => {
            if (err) {
              return reject(err);
            ...
            return this.scan(
              pattern,
              Number(newCursor),
              arrayOfKeys.concat(newArrayOfKeys)
            ).then(resolve);
          }
        );
      });
      ...
      return new Promise((resolve, reject) => {
        ...
        return client.keys(pattern, (err, keys) => {
          if (process.env.DEBUG_CACHE) {
            log(
              chalk.cyan(
                `Invalidating ${keys.length} keys for pattern: ${pattern}`
              )
            );
          }
          if (err) {
      ...
      return new Promise((resolve, reject) => {
        if (_.isFunction(value)) {
          ...
          if (result.isPending) {
            ...
            return result.then(resolve).catch((err) => reject(err));
          }
    ...
    getOrSet(key: string, value: any, ttl?: number): Promise<*> {
      return this.get(key).then((getResult) => {
        if (process.env.DEBUG_CACHE) {
